...
- **`ClausePurchaseOrderService::createOrder()`**: desa el `clause_value_at_order` actual i el `player_team_id`, amb el mateix escopament per lliga que la resta de l'app (un jugador rival d'una altra lliga mai pot generar una ordre). Just després de crear la fila, fa la mateixa comprovació en viu que `processPendingOrders()` — si la clàusula ja està desbloquejada en aquell mateix moment, no cal esperar el pròxim cicle programat (`fantasy:process-clause-orders`, fins a `FANTASY_SYNC_CLAUSE_ORDERS_FREQUENCY` minuts després); l'ordre pot acabar `EXECUTED`/`NEEDS_CONFIRMATION`/`FAILED` a la mateixa resposta de `POST /clause-orders`.
- **Disponible independentment de la recomanació i de si el jugador també està al mercat**: pagar una clàusula no depèn de cap `economicRecommendation` ni de si el propietari també l'ha posat en venda — al `PlayerController`, el botó/estat de l'ordre es calcula a partir de `owner`/`clauseValue` (sempre presents quan un rival el posseeix), mai del `context` (`OWNED_BY_RIVAL` vs `ON_MARKET`), que només decideix quina targeta de veredicte es mostra.
- **`processPendingOrders()`** (`fantasy:process-clause-orders`, cada `FANTASY_SYNC_CLAUSE_ORDERS_FREQUENCY` minuts — 5 per defecte, molt més sovint que els 120 de `fantasy:sync-clauses`, perquè aquí la rapidesa és tot el que importa) **mai es fia de l'últim sync** — a cada execució torna a llegir `getLeagueTeamRoster()` en viu per a cada ordre pendent:
  ...
  2. Preu actual superior a `clause_value_at_order` → `NEEDS_CONFIRMATION`, **mai paga sola** — cal que l'usuari confirmi el preu nou des de la fitxa del jugador o des de l'avís a `/` (Avui). Es comprova **abans** del bloqueig: una pujada de clàusula s'avisa immediatament encara que el jugador segueixi bloquejat.
  3. Clàusula encara bloquejada (`buyoutClauseLockedEndTime` futur) i preu no superior → es queda `PENDING`, es torna a provar al següent cicle.
  4. Desbloquejada i preu igual o inferior a `clause_value_at_order` → `payClause()` automàtic, `EXECUTED`.
  - Confirmar (`confirmAndExecute()`) **rearma** l'ordre al preu nou (`clause_value_at_order` = preu confirmat) i la torna a passar pel mateix flux: si segueix bloquejada queda `PENDING` esperant el desbloqueig, i si torna a pujar, torna a demanar confirmació.
...
- **Comprovació de saldo abans de pagar**: `execute()` (compartit per l'execució automàtica i per `confirmAndExecute()`) sempre crida `FantasyTeamService::getMoney()` **en viu** just abans de `payClause()` — mai es fia de `fantasy_teams.money`, que només és tan fresc com l'última `fantasy:sync-team` (cada 30 min per defecte, massa lent per a l'instant just abans de gastar diners reals). Si el saldo disponible és inferior a la clàusula, l'ordre queda `FAILED` amb un missatge clar ("No tens prou diners: calen X, en tens Y") sense arribar mai a trucar `payClause()` — no depèn d'endevinar com respon LaLiga a diners insuficients.
- **Si tot i així `payClause()` es rebutja per qualsevol altre motiu real** (el saldo pot haver canviat entre la comprovació i la crida, per exemple), l'app tampoc gasta res que no tingui: LaLiga mateix rebutja la crida, `execute()` no arriba mai a actualitzar l'ordre a `EXECUTED`, i queda `FAILED`. `describeFailure()` intenta mostrar el motiu real que torna l'API (buscant claus habituals `message`/`error`/`errorMessage`/`detail` al cos de la resposta) en lloc d'un genèric "rejected (400)" — el format exacte del cos d'error de `payClause()` no estava confirmat abans d'aquesta funcionalitat, així que si LaLiga fa servir una clau diferent, `error_message` cau al missatge genèric més el cos sencer en JSON, mai es perd la informació.
...
GET  /api/trading/build-xi?strategy=&horizon=      Trading · Construir onze
GET  /api/trading/make-money?horizon=&respect_reserve=   Trading · Guanyar diners
...
- **Trading** (`/trading`, dues pestanyes): centre d'estratègia econòmica, sense cap factor esportiu.
  - **Fonts**: valor i historial 1D/3D/7D de **FútbolFantasy** (`fantasy_external_trends`, mai barrejats amb snapshots propis ni amb `market_value` de LaLiga); estat de la lliga (caixa, plantilla, mercat, preu de sortida, comptador de pujes, clàusula, bloqueig/blindatge) de **LaLiga Fantasy**. Sense coincidència, sense cap finestra utilitzable o amb un valor FF que difereix >30% del de LaLiga (`fantasy.trading.value_divergence`) → candidat exclòs i marcat, mai inventat; una finestra que falta és `null`, mai 0.
  - **Cap càlcul econòmic nou**: projeccions, benefici, ROI, MaxBid i puja recomanada surten de `MarketBuyAnalysisService` / `ClauseEconomicAnalysisService` (mateixos pesos, decaïment i simulador de break-even). `TradingCandidateService` només munta el pool únic (propis / mercat / clàusules), el preu d'adquisició (puja recomanada ≥ preu de sortida i sempre ≤ MaxBid; la clàusula; 0 per als propis) i la confiança (coincidència, finestres, frescor, certesa del preu, coherència de valors — `fantasy.trading.confidence_weights`). Petits refactors compatibles: `PlayerValueTrendCalculator::externalHistoricalValues()`, paràmetre opcional `auctionReferenceValue` a `MarketBuyAnalysisService::analyze()` i `PlayerDecisionEngine::currentOffer()` públic.
  - **Construir onze** (`BuildXiOptimizer`): DP exacte per posició i formació; la formació és només una restricció de posicions (`fantasy.trading.formations`, **supòsit documentat**: el projecte no en coneixia cap fora de l'1-4-4-2 de `/team/lineup`). Estratègies: `max_return`, `balanced` (conserva `minimum_cash_reserve`), `min_cost`. Separa el benefici de les **noves adquisicions** de l'**evolució** dels jugadors que ja tens. Estats: `OK`, `ALREADY_HAVE_XI`, `IMPOSSIBLE_XI` (`MISSING_POSITION` / `INSUFFICIENT_CASH` amb el cost mínim per completar-lo).
  - **Guanyar diners** (`PortfolioOptimizer`, mochila de tria múltiple): ignora onze, posicions i mida de plantilla; maximitza el benefici absolut esperat a l'horitzó dins del capital; `KEEP_CASH` ("ara mateix no compraria res") és un resultat vàlid. Distingeix `currentCash` / `possibleCapitalFromSales` / `totalDeployableCapital`; un jugador propi només es ven si finança una compra millor o evita una pèrdua esperada. **Els preus de venda són estimacions etiquetades** (l'oferta real només s'usa si existeix una `FantasyOffer` pendent). Un jugador surt una sola vegada (mercat i clàusula competeixen pel mateix capital). El benefici mínim per moviment és el setting existent `trading_minimum_expected_profit`.
  - **Horitzons** 3D/7D/14D (defecte 14D, `fantasy.trading.horizons`); frescor de lliga/mercat/clàusules/FF amb llindars configurables (`fantasy.trading.staleness_minutes`) i avisos; resposta en memòria cau `fantasy.trading.cache_seconds` (la clau inclou cada marca de sincronització, la caixa i les regles). Sense migracions.
  - **Limitacions reals**: el matcher de noms descarta els ambigus sense persistir-los, així que "sense coincidència" i "ambigu" no es poden distingir; les ofertes de venda gairebé mai existeixen; la frescor de clàusules es dedueix dels snapshots de plantilles rivals.
