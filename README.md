...
**`pctChange24h`**: fins ara `TrendAnalysisService` només calculava el percentatge de variació a 3 i 7 dies (`pctChange3d`/`pctChange7d`) — les 24h només existien com a delta en euros (`change24h`), mai com a `%`. Calculat exactament igual (`(valorActual - valorFa24h) / valorFa24h × 100`) i afegit a `PlayerTrend`. `/team` ara té una columna **24H** entre Valor i Clàusula amb el mateix patró propi-preferit/extern-amb-fallback que ja fan servir totes les altres columnes de tendència (`trend.pctChange24h`; si és `null`, cau a `externalTrend.pct1d` amb l'etiqueta `*ext.`), i sota el `%` hi mostra també l'import real en euros (`trend.change24h`, o `externalTrend.delta1d` en fallback).
