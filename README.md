...
Un cop `install-windows.bat` ha acabat sense errors, `start-windows.bat` (arrel del repositori) arrenca l'app del dia a dia sense repetir cap comprovació d'instal·lació: obre dues finestres noves (`php artisan serve` al backend, `npm run dev` al frontend) i el navegador a `http://localhost:5173` al cap de 3 segons. Avisa i s'atura si `backend\vendor` o `frontend\node_modules` no existeixen (executa `install-windows.bat` primer) en lloc de deixar fallar `php`/`npm` amb un error críptic. Per tenir-lo a l'escriptori amb un sol clic: clic dret sobre el fitxer → "Enviar a" → "Escriptori (crear drecera)". Per aturar l'app, tanca les dues finestres de servidor que obre.
