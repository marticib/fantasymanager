...
Un cop `install-windows.bat` ha acabat sense errors, `fantasy_manager.bat` (arrel del repositori) arrenca l'app del dia a dia sense repetir cap comprovació d'instal·lació: obre tres finestres noves (`php artisan serve` al backend, `npm run dev` al frontend, `php artisan schedule:work` — vegeu més avall per què cal) i el navegador a `http://localhost:5173` al cap de 3 segons. Avisa i s'atura si `backend\vendor` o `frontend\node_modules` no existeixen (executa `install-windows.bat` primer) en lloc de deixar fallar `php`/`npm` amb un error críptic. Per tenir-lo a l'escriptori amb un sol clic: clic dret sobre el fitxer → "Enviar a" → "Escriptori (crear drecera)". Per aturar l'app, tanca les tres finestres que obre.

- **Sense `schedule:work` (o cron/Task Scheduler), l'app es queda sense dades**: confirmat amb un cas real — després de connectar el compte i triar la lliga, l'onboarding només desa la lliga/equip triats, no sincronitza res més (`FantasySyncService::selectLeague()` és deliberadament lleuger). Tota la resta — catàleg de jugadors, mercat, plantilla, clàusules de rivals, tendències externes — depèn dels 8 `Schedule::command(...)` de `routes/console.php`, i en producció (Linux) qui els dispara és el cron del sistema cridant `php artisan schedule:run` cada minut; **Windows no té cron**, i cap dels scripts d'aquest repositori arrencava mai res equivalent — per això la pàgina de Jugadors, Mercat, Avui, etc. es queden buides indefinidament encara que el login funcioni. `php artisan schedule:work` (el propi comando de Laravel per a desenvolupament local, sense dependre de cron) resol això mentre estigui obert; per a la primera càrrega de dades immediata, sense esperar el primer tick del planificador, executa manualment un cop `php artisan fantasy:sync && php artisan fantasy:sync-clauses && php artisan fantasy:sync-external-trends` des de `backend`.
...
207 tests, sense dependència de l'API real (`Http::fake()` a `FantasyApiClient`, `FantasyClauseService`; base de dades SQLite en memòria via `phpunit.xml`). Cobreixen: `FantasyApiClient` (401/refresh, 429, 5xx), `TrendAnalysisService`, `FantasyScoreService`, `MarketValueProjector`, `ClauseEconomicAnalysisService`, `ClauseOpportunityService`, `PlayerDecisionEngine`, `MarketBuyAnalysisService`, `MarketAuctionPremiumEstimator`, `TodayActionsService`, `PlayerController` (Feature — context OWNED_BY_ME/ON_MARKET/OWNED_BY_RIVAL/FREE), `HistoryController` (Feature — valor+saldo+backtesting), `FantasySnapshotDecisionsCommand` i `FantasyEvaluateDecisionsCommand` (Feature — sense look-ahead, sense recalcular amb config actual), el motor de recomanacions (compra/venda/mantenir + límit de capital), i `FantasySettingsService`.
